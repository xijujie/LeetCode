...
class Solution {
    public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
        ...
        return dfs(headID, informTime, children);
    }
    int dfs(int root, int[] time, List<List<Integer>> children) {
        int ans = 0;
        for (int i:children.get(root)){
            ans = Math.max(ans, dfs(i, time, children));
        }
        return ans + time[root];
    }
}
// @lc code=end
